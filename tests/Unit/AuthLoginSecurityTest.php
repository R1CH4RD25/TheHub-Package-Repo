...
class AuthLoginSecurityTest extends TestCase {
    ...
    public function testSuperAdminDoesNotMatchOtherRoleHelpers() {
        $_SESSION['logged_in'] = true;
        $_SESSION['user_id'] = 1;
        $_SESSION['role'] = 'super_admin';
        
        $this->assertTrue(Auth::isSuperAdmin());
        $this->assertFalse(Auth::isAdmin(), 'Super admin is not admin');
        $this->assertFalse(Auth::isManager(), 'Super admin is not manager');
        $this->assertFalse(Auth::isStaff(), 'Super admin is not staff');
    }
