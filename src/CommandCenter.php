<?php
/**
 * CommandCenter - High-level orchestration for submission management
 *
 * Provides dashboard stats, cross-section queries, and administrative
 * functions for the Command Center interface.
 *
 * @package Hub
 */
...
class CommandCenter
{
    ...
    public function getSectionsWithCounts($userId = null)
    {
        ...
        if ($userId) {
            $accessJoin = "INNER JOIN section_role_access sra ON s.id = sra.section_id";
            $accessWhere = "AND sra.user_id = ?";
            $params[] = $userId;
        }
        ...
        $sql = "SELECT 
                ...
                WHERE s.is_active = 1
                  {$accessWhere}
                GROUP BY s.id, s.name, s.slug, s.cc_prefix, s.icon
    ...
    public function getRecentSubmissions($limit = 10, $userId = null)
    {
        ...
        if ($userId) {
            $accessJoin = "INNER JOIN section_role_access sra ON s.section_id = sra.section_id 
                           AND sra.user_id = ?";
            $params[] = $userId;
        }
